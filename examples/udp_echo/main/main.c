...
#define EXAMPLE_SOCKET_NUM       1
#define EXAMPLE_LOCAL_PORT       5001
#define EXAMPLE_REMOTE_PORT      5001
#define EXAMPLE_IO_TIMEOUT_MS    5000

static const char *TAG = "udp_echo_example";
static const uint8_t EXAMPLE_REMOTE_IP[4] = {192, 168, 1, 10};
#if !EXAMPLE_USE_DHCP
static const uint8_t EXAMPLE_STATIC_IP[4] = {192, 168, 1, 70};
...
static void fill_default_config(esp_wiz_toe_config_t *cfg)
{
    ...
    cfg->timeout.connect_timeout_ms = EXAMPLE_IO_TIMEOUT_MS;
...
static void udp_echo_task(void *arg)
{
    ...
    while (true) {
        ...
        esp_err_t ret = esp_wiz_toe_socket(
            EXAMPLE_SOCKET_NUM,
            ESP_WIZ_TOE_PROTO_UDP,
            EXAMPLE_LOCAL_PORT,
            0,
            EXAMPLE_IO_TIMEOUT_MS);
        if (ret != ESP_OK) {
            ESP_LOGW(TAG, "UDP open failed: %s", esp_err_to_name(ret));
            vTaskDelay(pdMS_TO_TICKS(1000));
            continue;
        }
        ...
        ret = esp_wiz_toe_udp_sendto(
            ...
            &sent_len);
        if (ret != ESP_OK) {
            ESP_LOGW(TAG, "UDP sendto failed: %s", esp_err_to_name(ret));
            (void)esp_wiz_toe_close(EXAMPLE_SOCKET_NUM);
            vTaskDelay(pdMS_TO_TICKS(1000));
            continue;
        }
        ESP_LOGI(TAG, "UDP sent %u bytes", (unsigned)sent_len);
        ...
        if (ret == ESP_OK) {
            rx_buf[recv_len] = '\0';
            ESP_LOGI(TAG,
                     "UDP RX %u bytes from %u.%u.%u.%u:%u -> %s",
                     (unsigned)recv_len,
                     src_ip[0],
                     src_ip[1],
                     src_ip[2],
            ...
            ESP_LOGW(TAG, "UDP recvfrom timeout/error: %s", esp_err_to_name(ret));
        }
...
void app_main(void)
{
    xTaskCreate(udp_echo_task, "udp_echo_task", 4096, NULL, 5, NULL);
}
